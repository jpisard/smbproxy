...
""".replace(' ', '').replace('\n', ''))
        ...
        def createCB(create_message, **kwargs):
            messages_history.append(create_message)
            if create_message.status == 0:
                sendWrite(create_message.tid, create_message.payload.fid, 0L)
            else:
...
""".replace(' ', '').replace('\n', ''))
        ...
        def createCB(create_message, **kwargs):
            ...
            if create_message.status == 0:
                ...
                errback(OperationFailure('Failed to create directory %s on %s: Create failed with status %d' % ( path, service_name, create_message.status ), messages_history))
